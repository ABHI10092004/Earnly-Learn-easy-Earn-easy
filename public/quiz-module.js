...
async function initializeQuizModule() {
    try {
        ...
        console.error('Error initializing quiz module:', error);
        showError('Failed to load quiz module. Please try again later.');
    }
}
...
async function fetchModuleDetails(moduleId) {
    try {
        ...
        console.log('Fetching quiz module details:', { courseId, moduleId });
        ...
        // Make sure quiz questions exist
        if (!currentModule.content || !currentModule.content.questions || !Array.isArray(currentModule.content.questions)) {
            console.error('Quiz questions are missing or invalid format');
            throw new Error('This quiz has no questions or is in an invalid format');
        }
    } catch (error) {
        console.error('Error in fetchModuleDetails:', error);
        showError(error.message || 'Failed to load quiz details');
        throw error;
    }
}
    ...
    try {
        ...
        if (currentModule && userProgress.moduleProgress) {
            ...
            if (moduleProgress && moduleProgress.isCompleted) {
                console.log('Current quiz module is already completed');
...
function updateModuleUI() {
    ...
    if (moduleReward) moduleReward.textContent = `Reward: ${currentModule.reward || '0'}`;
    if (rewardAmount) rewardAmount.textContent = currentModule.reward || '0';
    ...
    if (userProgress && currentModule) {
        ...
        if (moduleProgress && moduleProgress.isCompleted) {
            if (submitQuizBtn) {
                ...
                submitQuizBtn.innerHTML = '<i class="fas fa-check-circle me-2"></i>Completed';
            }
            isQuizSubmitted = true;
        }
    }
...
function renderQuizQuestions() {
    if (!currentModule?.content?.questions) {
        quizQuestions.innerHTML = '<div class="alert alert-danger">No questions available for this quiz module.</div>';
        return;
    }

    const questionsHTML = currentModule.content.questions.map((question, index) => `
...
function setupEventListeners() {
    ...
    // Submit quiz button
    if (submitQuizBtn) {
        submitQuizBtn.addEventListener('click', handleQuizCompletion);
    }
...
function updateQuizProgress() {
    if (!quizProgressBar || !currentModule?.content?.questions) return;
    
    const totalQuestions = currentModule.content.questions.length;
...
async function handleQuizCompletion() {
    ...
    if (isQuizSubmitted) {
        console.log('Quiz already submitted, returning early');
        return;
    }
    ...
    try {
        ...
        if (answeredQuestions < totalQuestions) {
            showError(`Please answer all questions (${answeredQuestions}/${totalQuestions} answered)`);
            return;
        }
        ...
        if (submitQuizBtn) {
            submitQuizBtn.disabled = true;
            submitQuizBtn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Submitting...';
        }
        
        const score = calculateScore();
        console.log(`Quiz score calculated: ${score}%`);
        
        console.log('Submitting quiz completion:', { courseId, moduleId, score });
        ...
        let successMessage = `Quiz completed with score: ${Math.round(score)}%`;
        if (data.reward) {
            successMessage = `Quiz completed! You earned ${data.reward} coins. Score: ${Math.round(score)}%`;
        }
        ...
        if (currentIndex >= 0 && currentIndex < currentCourse.modules.length - 1) {
            ...
            if (nextModule) {
                ...
                showSuccess(`Quiz completed! Next up: "${nextModuleTitle}"`);
                ...
                setTimeout(() => {
                    ...
                    navigateModule('next');
                }, 2500); // Slightly longer delay for quiz to allow user to see their score
            }
        } else {
        ...
        showError(error.message || 'Failed to submit quiz results. Please try again.');
        
        // Reset button state
        if (submitQuizBtn) {
            console.log('Resetting button state after error');
            submitQuizBtn.disabled = false;
            submitQuizBtn.innerHTML = '<i class="fas fa-check-circle me-2"></i>Submit Quiz';
        }
    }
}
...
function calculateScore() {
    if (!currentModule?.content?.questions) return 0;

    let correctAnswers = 0;
    currentModule.content.questions.forEach((question, index) => {
        if (selectedAnswers[index] === question.correctAnswer) {
            correctAnswers++;
        }
    });

    return (correctAnswers / currentModule.content.questions.length) * 100;
}
...
function showQuizResults(score) {
    ...
    if (score >= 80) {
        ...
        resultDescription.textContent = 'You have a great understanding of this topic!';
    } else if (score >= 60) {
        resultMessage.textContent = 'Good job!';
        resultDescription.textContent = 'You have a good understanding of this topic.';
    } else {
        resultMessage.textContent = 'Keep practicing!';
        resultDescription.textContent = 'You might want to review this topic again.';
    }
...
// Initialize the quiz module when the page loads
document.addEventListener('DOMContentLoaded', initializeQuizModule); 